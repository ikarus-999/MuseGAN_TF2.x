...
class MuseGAN():
    def __init__(self
        ...
        self.weight_init = GlorotNormal() #RandomNormal(mean=0., stddev=0.02) # 'he_normal' #RandomNormal(mean=0., stddev=0.02)
    ...
    def gradient_penalty_loss(self, y_true, y_pred, averaged_samples):
        ...
        averaged_samples = tf.clip_by_value(y_pred, 1e-6, 1.)
...
#         return K.mean(gradient_penalty)
    ...
    def wasserstein(self, y_true, y_pred):
        return -K.mean(y_true * tf.clip_by_value((y_pred), 1e-6, 1.))
    ...
    def conv(self, x, f, k, s, a, p):
        x = tfa.layers.SpectralNormalization(Conv3D(
                    filters = f
                    , kernel_size = k
                    , padding = p
                    , strides = s
    ...
    def _build_critic(self):
        ...
        label_input = Input(shape=self.input_dim[:-1] + (1,), name='label_c_input')
        
        x = tf.math.multiply(critic_input, label_input)
        ...
        #y = label_input
        #y = self.conv(y, f=128, k = (2,1,1), s = (1,1,1), a = 'lrelu', p = 'valid')
        #y = self.conv(y, f=128, k = (self.n_bars - 1,1,1), s = (1,1,1), a = 'lrelu', p = 'valid')
        
        #y = self.conv(y, f=128, k = (1,1,12), s = (1,1,12), a = 'lrelu', p = 'same')
        #y = self.conv(y, f=128, k = (1,1,7), s = (1,1,7), a = 'lrelu', p = 'same')
        #y = self.conv(y, f=128, k = (1,2,1), s = (1,2,1), a = 'lrelu', p = 'same')
        #y = self.conv(y, f=128, k = (1,2,1), s = (1,2,1), a = 'lrelu', p = 'same')
        #y = self.conv(y, f=256, k = (1,4,1), s = (1,2,1), a = 'lrelu', p = 'same')
        #y = self.conv(y, f=512, k = (1,3,1), s = (1,2,1), a = 'lrelu', p = 'same')
    ...
    def conv_t(self, x, f, k, s, a, p, bn):
        x = tfa.layers.SpectralNormalization(Conv2DTranspose(
                    filters = f
                    , kernel_size = k
                    , padding = p
                    , strides = s
    ...
    def _build_generator(self):
        ...
        label_input = Input(shape=(self.input_dim[:-1] + (1,)), name='label_input')
        ...
        #print("???")
        #print('g', generator_output.shape)
        #print('y', label_input.shape)
        generator_output = tf.math.multiply(generator_output, label_input)
        #print("!!! ")
        
        self.generator = Model([chords_input, style_input, melody_input, groove_input, label_input], generator_output)
    ...
    def _build_adversarial(self):
        ...
        melody_input = Input(shape=(self.n_tracks, self.z_dim), name='melody_input')
        groove_input = Input(shape=(self.n_tracks, self.z_dim), name='groove_input')
        label_input = Input(shape=(self.input_dim[:-1] + (1,)), name='label_y_input')

        fake_img = self.generator([chords_input, style_input, melody_input, groove_input, label_input])
        # mix_img = Multiply()([fake_img, label_input])
        # critic determines validity of the real and fake images
        fake = self.critic([fake_img, label_input])
        valid = self.critic([fake_img, label_input])
        
        # Construct weighted average between real and fake images
        interpolated_img = RandomWeightedAverage(self.batch_size)([real_img, fake_img])
        # Determine validity of weighted sample
        validity_interpolated = self.critic([interpolated_img, label_input])
        ...
        self.critic_model = Model(inputs=[real_img, chords_input, style_input, melody_input, groove_input, label_input],
        ...
        melody_input = Input(shape=(self.n_tracks, self.z_dim), name='melody_input')
        groove_input = Input(shape=(self.n_tracks, self.z_dim), name='groove_input')
        y_input = Input(shape=(self.input_dim[:-1] + (1,)), name='label_y_input')

        # Generate images based of noise
        img = self.generator([chords_input, style_input, melody_input, groove_input, y_input])
        
        # Discriminator determines validity
        model_output = self.critic([img, y_input])
        # Defines generator model
        self.model = Model([chords_input, style_input, melody_input, groove_input, y_input], model_output)
    ...
    def train_critic(self, x_train, batch_size, using_generator, cond_idx = 0):
        ...
        # print(true_imgs.shape) #  (B, 2, 16, 84, 4)
        
        label = np.expand_dims(true_imgs[:, :, :, :, cond_idx], axis=-1)

        d_loss = self.critic_model.train_on_batch([true_imgs, chords_noise, style_noise,melody_noise,groove_noise, label], [valid, fake, dummy])
        return d_loss

    def train_generator(self, x_train, batch_size, using_gen, cond_idx = 0):
        valid = np.ones((batch_size,1), dtype=np.float32)
        ...
        groove_noise = np.random.normal(0, 1, (batch_size, self.n_tracks, self.z_dim))
        label = np.expand_dims(true_imgs[:, :, :, :, cond_idx], axis=-1)
        # print(label.shape) #(64, 2, 16, 84, 1)
        g_loss = self.model.train_on_batch([chords_noise, style_noise,melody_noise,groove_noise, label], valid)
        return g_loss
    ...
    , using_generator = False):
        ...
        for epoch in range(self.epoch, self.epoch + epochs):
            ...
            g_loss = self.train_generator(x_train, batch_size, using_generator)
            ...
            if epoch % print_every_n_batches == 0:
                # self.sample_images(run_folder, batch_size, x_train, using_generator)
    ...
    def sample_images(self, run_folder, batch_size, x_train, using_gen, cond_idx = 0):
        r = 5
        ...
        label = np.expand_dims(true_imgs[:, :, :, :, cond_idx], axis=-1)
        #y = np.random.normal(0, 1, (r, self.batch_size, self.) )
        # print(true_imgs.shape)
        y = np.expand_dims(true_imgs[:r, :, :, :, cond_idx], axis=-1)
        
        #print('y? ', y.shape)
        # print(groove_noise.shape)
        
        gen_scores = self.generator.predict([chords_noise, style_noise, melody_noise, groove_noise, label], verbose=1)
        #print('z', gen_scores.shape)
