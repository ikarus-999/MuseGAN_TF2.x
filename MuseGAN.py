...
class MuseGAN():
    ...
    def _build_critic(self):
        cond_input = Input(shape=(self.input_dim[:-1]+(1,)), name='cond_input')

        critic_input = Input(shape=(self.input_dim), name='critic_input')

        x = Concatenate(axis=-1)([cond_input, critic_input])
        ...
        self.critic = Model([cond_input, critic_input], critic_output)
    ...
    def _build_generator(self):
        ...
        out = Concatenate(axis = 1, name = 'concat_bars')(bars_output)

        #print(out.shape)
        #print(cond_input.shape)

        generator_output = Multiply()([out, cond_input])
        
        self.generator = Model([cond_input, chords_input, style_input, melody_input, groove_input], generator_output)
    ...
    def _build_adversarial(self):
        ...
        fake_img = self.generator([cond_input, chords_input, style_input, melody_input, groove_input])

        # critic determines validity of the real and fake images
        fake = self.critic([cond_input, fake_img])
        valid = self.critic([cond_input, real_img])

        # Construct weighted average between real and fake images
        interpolated_img = RandomWeightedAverage(self.batch_size)([real_img, fake_img])
        # Determine validity of weighted sample
        validity_interpolated = self.critic([cond_input, interpolated_img])
        ...
        self.critic_model = Model(inputs=[cond_input, real_img, chords_input, style_input, melody_input, groove_input],
        ...
        img = self.generator([cond_input, chords_input, style_input, melody_input, groove_input])
        # Discriminator determines validity
        model_output = self.critic([cond_input, img])
        # Defines generator model
        self.model = Model([cond_input, chords_input, style_input, melody_input, groove_input], model_output)
    ...
    def train_critic(self, cond_input, x_train, batch_size, using_generator):
        ...
        d_loss = self.critic_model.train_on_batch([cond_input, true_imgs, chords_noise, style_noise,melody_noise,groove_noise], [valid, fake, dummy])
        return d_loss

    def train_generator(self, cond_input, batch_size):
        valid = np.ones((batch_size,1), dtype=np.float32)
        ...
        return self.model.train_on_batch([cond_input, chords_noise, style_noise,melody_noise,groove_noise], valid)


    def train(self, cond_input, x_train, batch_size, epochs, run_folder, print_every_n_batches = 10
    , n_critic = 5
    , using_generator = False):
        ...
        for epoch in range(self.epoch, self.epoch + epochs):
            ...
            for _ in range(critic_loops):
                d_loss = self.train_critic(cond_input, x_train, batch_size, using_generator)

            g_loss = self.train_generator(cond_input, batch_size)
            ...
            if epoch % print_every_n_batches == 0:
                self.sample_images(cond_input, run_folder)
    ...
    def sample_images(self, cond_input, run_folder):
        r = 5
        ...
        cond_input = cond_input[:r, :, :, :, :]
        gen_scores = self.generator.predict([cond_input, chords_noise, style_noise, melody_noise, groove_noise])
