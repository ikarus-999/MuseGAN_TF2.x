...
from module1 import BarEncoder, BarGenerator
...
class MuseGAN():
    ...
    def _build_generator(self):
        ...
        # N, 4, 16, 84, 1
        acc_track = Input(shape=(2*48, self.n_pitches, 1), name='condition_input')
        BE = BarEncoder()
        nowbar = BE(cond_tensor=acc_track)
        ...
        for bar in range(self.n_bars):
            ...
            for track in range(self.n_tracks):
                ...
                temp_track = self.barGen[track](in_tensor=z_input, nowbar=nowbar) # , (-1, 1, self.n_steps_per_bar,  self.n_pitches, 1)
                track_output[track] = tf.reshape(temp_track, (-1, 1, self.n_steps_per_bar, self.n_pitches, 1))

                # print(np.shape(track_output[track])) # (None, 96, 84, 1)

            bars_output[bar] = Concatenate(axis = -1)(track_output)
        
        generator_output = Concatenate(axis = 1, name = 'concat_bars')(bars_output)
        print(f'gen_output: {generator_output.shape}')
        self.generator = Model([chords_input, style_input, melody_input, groove_input, acc_track], generator_output)
    ...
    def _build_adversarial(self):
        ...
        real_img = Input(shape=(self.input_dim), name='real_img_input')
        ...
        fake_img = self.generator([chords_input, style_input, melody_input, groove_input, acc_track])

        # critic determines validity of the real and fake images
        fake = self.critic(fake_img)
        valid = self.critic(real_img)
        print(f'fake: {fake_img.shape}, real: {real_img.shape}')
        ...
        self.critic_model = Model(inputs=[real_img, chords_input, style_input, melody_input, groove_input, acc_track],
        ...
        groove_input = Input(shape=(self.n_tracks, self.z_dim), name='groove_input')
        # (48*2, 84, 1) -> (2, 48, 84, 1)
        # acc_track = Input(shape=( 48, self.n_pitches, 1), name='condition_input')

        # Generate images based of noise
        img = self.generator([chords_input, style_input, melody_input, groove_input, acc_track])
        # Discriminator determines validity
        model_output = self.critic(img)
        # Defines generator model
        self.model = Model([chords_input, style_input, melody_input, groove_input, acc_track], model_output)
    ...
    def train_critic(self, x_train, batch_size, using_generator):
        ...
        # print(true_imgs.shape) # (64, 4, 32, 84, 5)
        tmp_cond = tf.expand_dims(true_imgs[..., 1], -1)
        len_x = true_imgs.shape[0]
        print(f'tmp0: {tmp_cond.shape}')

        tmp_cond = tf.slice(tmp_cond, [0, 0, 0, 0, 0], [-1, 3, 32, -1, -1])
        # print(f'tmp: {tmp_cond.shape}')
        acc_track = tf.reshape(tmp_cond, (len_x, -1, self.n_pitches, 1))
        print(f'fake: {fake.shape}')
        print(f'true: {true_imgs.shape}')
        print(f'acc : {acc_track.shape}')

        d_loss = self.critic_model.train_on_batch([true_imgs, chords_noise, style_noise,melody_noise,groove_noise, acc_track], [valid, fake, dummy])
        return d_loss

    def train_generator(self, x_train, batch_size, using_generator):
        valid = np.ones((batch_size,1), dtype=np.float32)
        ...
        tmp_cond = tf.slice(tmp_cond, [0, 0, 0, 0, 0], [-1, 3, 32, -1, -1])
        acc_track = tf.reshape(tmp_cond, (len_x, -1, self.n_pitches, 1))
        print(f'acc_2{acc_track.shape}')

        return self.model.train_on_batch([chords_noise, style_noise,melody_noise,groove_noise, acc_track], valid)
    ...
    , using_generator = False):
        ...
        #acc_y = tf.slice(x_train, [0, 0, 0, 0], [-1, 96, -1, 1]) #x_train[:, :96, :84, :1]  #tf.slice(x_train, [0, 0, 0, 0], [-1, -1, -1, 1])
        ...
        for epoch in range(self.epoch, self.epoch + epochs):
            ...
            g_loss = self.train_generator(x_train, batch_size)
            ...
            line1, = plt.plot(range(epoch+1), self.d_losses, 'r')
            line2, = plt.plot(range(epoch+1), self.g_losses, 'k')
